...
public class ExternalHtmlParseFilter implements ParseFilter {
    ...
    public Parse filter(String url, WebPage page, Parse parse,
        HTMLMetaTags metaTags, DocumentFragment documentFragment) {

        DocumentFragment rootToIndex;
        StringBuilder strippedContent = new StringBuilder();
        if (whiteListSelectors.size() > 0 && !protectedURLs.contains(page.getBaseUrl())) {
            rootToIndex = (DocumentFragment) documentFragment.cloneNode(false);
            whitelisting(documentFragment, rootToIndex);
        } else if (blackListSelectors.size() > 0 && !protectedURLs.contains(page.getBaseUrl())) {
        ...
        String content = strippedContent.toString();
        
        LOG.info("Parse-Content: " + content);

        CharsetEncoder encoder = Charset.forName("UTF-8").newEncoder();
        try {
           page.getMetadata().put(new Utf8("filtered-content"),
              encoder.encode(CharBuffer.wrap(content)));
    ...
    public void setConf(Configuration conf) {
        ...
        String elementsToExclude = getConf().get("parser.filter.blacklist", null);
        ...
        String elementsToInclude = getConf().get("parser.filter.whitelist", null);
        ...
        this.protectedURLs = new HashSet<String>(Arrays.asList(getConf().get("parser.filter.protected_urls", "").split(",")));
    }
